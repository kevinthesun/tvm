...
@script
def _max_pool2d_shape_func(data_shape, pool_size, strides,
                           padding, height_axis, width_axis):
    out = output_tensor((data_shape.shape[0],), "int64")
    for i in const_range(data_shape.shape[0]):
        if i == height_axis:
            out[i] = (data_shape[i] + 2 * padding[0] - pool_size[0]) // strides[0] + 1
        elif i == width_axis:
            out[i] = (data_shape[i] + 2 * padding[1] - pool_size[1]) // strides[1] + 1
        else:
            out[i] = data_shape[i]

    return out

@reg.register_shape_func("nn.max_pool2d", False)
def max_pool2d_shape_func(attrs, inputs, _):
    ...
    return [_max_pool2d_shape_func(inputs[0], convert(pool_size),
                                   convert(strides), convert(padding),
                                   convert(height_axis), convert(width_axis))]

@reg.register_shape_func("nn.bias_add", False)
def bias_add_shape_func(attrs, inputs, _):
    return [inputs[0]]
...
@reg.register_shape_func("nn.global_max_pool2d", False)
def global_max_pool2d_shape_func(attrs, inputs, _):
    layout = attrs.layout
...
@reg.register_shape_func("nn.global_avg_pool2d", False)
def global_max_pool2d_shape_func(attrs, inputs, _):
    layout = attrs.layout
    height_axis = width_axis = 1
    for i, letter in enumerate(layout):
